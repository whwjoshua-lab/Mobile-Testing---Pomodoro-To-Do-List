...
document.addEventListener('click', (e) => {
    const target = e.target.closest('button, .nav-btn, .pixel-btn, .theme-card, .task-card, select, .intro-btn, .model-btn, .unit-btn, .num-btn, .preset-btn');
    if (target) {
...
// --- Intro Page Logic ---
const modelSelectionPage = document.getElementById('model-selection-page');
...
function createTaskElement(text, parent) {
    ...
    div.addEventListener('touchmove', (e) => {
        ...
        div.style.display = 'block'; // Keep it visible while dragging
        div.style.pointerEvents = 'none'; // Allow elementFromPoint to see through

        // Highlight potential drop target
        const elemBelow = document.elementFromPoint(touch.clientX, touch.clientY);
        if (elemBelow) {
            const droppableBelow = elemBelow.closest('.column');
            // Optional: Add visual feedback to droppableBelow
        }
    }, { passive: false });

    div.addEventListener('touchend', (e) => {
        const touch = e.changedTouches[0];
        div.style.display = 'none'; // Hide briefly to check what's underneath
        const elemBelow = document.elementFromPoint(touch.clientX, touch.clientY);
        div.style.display = 'block';
